    ...
    public void execute(ClientHandler handler, Message msg) throws Exception {
        ...
        if (OnlineUserManager.isOnline(username)) {
            ...
            if (!(username.equals("admin") && password.equals("1234"))) {
                handler.sendMessage(new Message("LOGIN_RESPONSE", "SERVER",
                        new Status(StatusType.ERROR, "Password or username is incorrect!")));
                return;
            }
            handler.setUsername(username);
            OnlineUserManager.addOnlineUser(username, handler);
            handler.sendMessage(
                    new Message("LOGIN_RESPONSE", "SERVER", new Status(StatusType.SUCCESS, "Login success")));
            for (ClientHandler h : OnlineUserManager.getAllHandlers()) {
                if (h != handler) {
                    h.sendMessage(new Message("USER_ONLINE", "SERVER", username)); // will be converted to user's
                                                                                   // ingame.
                }
            }
        }
